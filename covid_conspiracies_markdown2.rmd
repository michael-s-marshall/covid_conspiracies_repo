...
results <- left_join(
  exp(coef(dist_full)) %>% as_tibble() %>% 
    gather(variable,odds_ratio,`(Intercept)`:W1_Conspiracy_Total) %>% 
    mutate(response = rep(c("Agree","Neither","Disagree",
                            "Strongly disagree"),16)),
  p_value(dist_full) %>% as_tibble() %>% 
...
library(knitr)
kable(results, caption = "Multinomial Regression Results for Social Distancing: 'wanted to do it'")
```
...
```{r, echo = FALSE, results = 'asis'}
results <- left_join(
  exp(coef(dist12_full)) %>% as_tibble() %>% 
    gather(variable,odds_ratio,`(Intercept)`:W1_Conspiracy_Total) %>% 
    mutate(response = rep(c("Agree","Neither","Disagree",
                            "Strongly disagree"),16)),
  p_value(dist12_full) %>% as_tibble() %>% 
    gather(variable,p_value,`(Intercept)`:W1_Conspiracy_Total) %>% 
    mutate(response = rep(c("Agree","Neither","Disagree",
                            "Strongly disagree"),16)),
  by = c("variable","response")
...
kable(results, caption = "Multinomial Regression Results for Social Distancing: 'good thing to do'")
```
