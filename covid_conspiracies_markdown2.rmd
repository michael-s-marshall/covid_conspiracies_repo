...
results <- left_join(
  coef(vax_full) %>% as_tibble() %>% 
    gather(variable,coefficient,`(Intercept)`:W1_Conspiracy_Total) %>% 
    mutate(response = rep(c("No","Maybe"),16)),
  sum_table$standard.errors %>% as_tibble() %>% 
    gather(variable, std_error,`(Intercept)`:W1_Conspiracy_Total) %>% 
    mutate(response = rep(c("No","Maybe"),16)),
...
Note: trust in science (W2_Trust_Body6) removed, as may be tapping into a latent 'distrustful' disposition invaldating the exogeneity assumption.  
