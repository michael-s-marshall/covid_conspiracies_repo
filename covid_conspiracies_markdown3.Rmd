...
The code below turns the **preferred newspaper** variables into dummy variables, as they were previously coded as **1=Yes** and everything else as **NA**, so the **NA** is recoded as a **0**.  
...
Scales for a number of dependent variables are created using the **psych** package.  Some of the scales already have already been calculated in the dataset, yet they are reproduced below to check there are no errors.  The dependent variables produced are listed below, with the variables names in **bold** and whether it is a produced scale or one pre-existing in the dataset indicated in brackets:  

* ethnocentrism, **ethno** (produced)  
* right-wing authoritarianism, **RWA** (produced)  
* social dominance orientation, **SDO** (produced)  
* conspiracy mentality, **W1_Conspiracy_Total** (pre-existing)  
* intolernce of uncertainty, **W2_IOU_Total** (pre-existing)  
...
Below are a number of variables that are scaled between 0-1. Ordinal variables are treated as pseudo-continuous. They are:  

* COVID-19 anxiety, **threat**  
* left-right self-placement, **right**  
* age, **age_sc**  
* distrust of scientists, **distrust_science**  
* degree of information from social media, **W2_INFO_5**  
* degree of information from family and friends, **W2_INFO_9**  
* household income, **W1_Income_2019**  
* conspiracy mentality, **W1_Conspiracy_Total**  
* intolerance of uncertainty, **W2_IOU_Total**  
...
Ethnicity and gender are turned to factors below, as opposed to character strings.  
...
Finally, each conspiracy belief scale (i.e. the dependent variables) is also scaled between 0-1.  
...
A for loop to look at distribution of independent variables (numeric only).  
...
Correlation between the origin theories below. Statistical significance tested via kendall's tau, with bonferroni correction for 3 tests.  
...
Functions below are for the IHS transformation, including estimation of the concentrated log likelihood in order to estimate parameter **theta**.  Adapted from [Burbidge, Magee and Robb (1988)](https://www.jstor.org/stable/2288929?seq=1).  
...
Code below seeks to optimize the concentrated log likelihood, thus returning the best theta.  
...
Theta continues to rise indefinitely, therefore using **asinh** function, equivalent to log(x + (x^2 + 1)^(1/2)).  As shown by the distributions below, this reduces the long-tail of the data, without producing what becomes essentially a binary outcome as theta increases.  
...
Below are coefficient plots for each of the models predicting origin thoery belief.  First groups of variables are set up for panels in the plots.  
...
Below is a combined plot of the model coefficients for conspiracy mentality and predispositions.  
...
Below is a combined plot of model predictions on conspiracy mentality and predispositions for the supplementary models (IHS and poisson).  
...
A scale is produced for social distancing motivation below.  And then OLS regression performed.  NAs are removed leaving n=1399.  
...
Average marginal effects produced using the **margins** package, with bootstrapped standard errors.  
...
Relative risk ratios are produced below, with 95% confidence intervals and coefficient plots.  
...
Below a table is produced for the means and standard deviations of study variables.  
...
Correlation matrix of study variables, using pearman's correlation coefficient and bonferroni adjusted p-values.  
